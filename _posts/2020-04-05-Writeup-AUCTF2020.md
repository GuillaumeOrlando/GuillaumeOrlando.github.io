...
La deuxième clé est obtenu grâce à la fonction _check2_, qui inverse l'ordre des lettres d'une chaine passée en entrée:
...
Bien que ce challenge puisse être flag assez rapidement, j'aimerai m'attarder sur l'implémentation de la technique anti-re employée par l'auteur de ce challenge.
...
Ces deux instructions ont donc comme objectif de mettre l'adresse de l'instruction '_pop edi_' dans le registre _EDI_, à savoir l'adresse  _0x1235_.
...
Viens ensuite une boucle, qui va incrémenter de 1 byte _EDI_, de façon à parcourir une plage d'adresses à la recherches de breakpoint, pendant 0x400 instructions:
...
La fonction encrypt semble prendre notre input, la modifier, avant de la comparer avec une version modiffée du mot de passe:
...
En reconstituant le puzzle ('A' = 'M', 'B' = 'D', etc ...), le mot de passe est découvert sans trop d'efforts: La chaine chiffrée "_SASRRWSXBIEBCMPX_" correspond à "_IKILLWInHMYHEAln_".
...
Il est cependant possible, avec un peu d'expérience, de reconnaitre la structure d'un fichier binaire dans ce fichier, notamment entre les différentes sections entrecoupées de '_0x0_' (header, section .text, .data, etc ...).
...
L'exécutable fournis est ici un binaire au format PE32, histoire de changer de plateforme.
...
En observant linéairement les instructions exécuté depuis le main, il est possible de remarquer que le programme commence sa routine par en créant un '[exception handler](https://docs.microsoft.com/en-us/windows/win32/debug/structured-exception-handling)':
...
Le 'mov' indique que le premier élément de la chaine d'instruction est désormais la fonction référencée par le premier _push_.
...
En essayant avec les paramètres de connextion donnée avec l'énoncé du challenge, rien ne se passe:
...
Le problème vient simplement du fait que le programme envoi la seconde requête sensé récupérer le flag, mais ne fait rien avec la réponse.
